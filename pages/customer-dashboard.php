    ...
    <div class="container">
        ...
        <div class="dashboard-top">
            ...
            <div class="customer-hero-actions">
                ...
                <a class="btn btn-ghost" href="<?= e(page_url('chat', ['new' => 'support'])) ?>"><i class="bi bi-life-preserver me-1"></i>Bantuan</a>
            </div>
        </div>

        <div class="dashboard-grid customer-actions-grid">
            <a href="<?= e(page_url('orders')) ?>" class="dashboard-action glass-card">
                <i class="bi bi-bag-check"></i>
                <div><strong><?= count($orders) ?> pesanan</strong><small><?= $open ?> berjalan · <?= $completed ?> selesai<?php if ($cancelled): ?> · <?= $cancelled ?> dibatalkan<?php endif; ?></small></div>
                <i class="bi bi-arrow-right arrow"></i>
            </a>
            <a href="<?= e(page_url('cart')) ?>" class="dashboard-action glass-card">
                <i class="bi bi-cart3"></i>
                <div><strong>Keranjang</strong><small>Periksa item sebelum checkout</small></div>
                <i class="bi bi-arrow-right arrow"></i>
            </a>
            <a href="<?= e(page_url('chat')) ?>" class="dashboard-action glass-card">
                <i class="bi bi-chat-dots"></i>
                <div><strong>Chat & pengaduan</strong><small>Hubungi seller atau admin</small></div>
                <i class="bi bi-arrow-right arrow"></i>
            </a>
            <a href="<?= e(page_url('profile')) ?>" class="dashboard-action glass-card">
                <i class="bi bi-person"></i>
                <div><strong>Profil saya</strong><small>Kelola data akun</small></div>
                <i class="bi bi-arrow-right arrow"></i>
            </a>
        </div>
        ...
        <div class="customer-products-panel glass-card mt-4">
            <div class="customer-products-head">
                <div>
                    ...
                    <h2>Barang terbaru <em>buat kamu.</em></h2>
                    <p>Pilihan produk aktif dengan stok tersedia.</p>
                </div>
                ...
                <?php foreach ($products as $p): ?>
                    <article class="customer-product-card">
                        <a class="customer-product-media" href="<?= e(page_url('product-detail', ['id' => $p['id']])) ?>">
                            ...
                            <?php else: ?>
                                <span class="customer-product-placeholder"><i class="bi bi-box-seam"></i></span>
                            <?php endif; ?>
                            <span class="customer-product-badge"><?= e((string) $p['category']) ?></span>
                            <span class="customer-product-condition"><?= e((string) $p['condition_label']) ?></span>
                        </a>
                        <div class="customer-product-body">
                            ...
                            <div class="customer-product-bottom">
                                ...
                                <a class="btn btn-soft btn-sm" href="<?= e(page_url('product-detail', ['id' => $p['id']])) ?>">Lihat</a>
                            </div>
                        </div>
                    </article>
                <?php endforeach; ?>
