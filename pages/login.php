...
if (is_post()) {
    ...
    // Validasi reCAPTCHA tetap dijalankan untuk setiap percobaan login,
    // tetapi pesan kredensial diprioritaskan agar user tidak disesatkan.
    if (!$errors) {
        ...
        $recaptchaValid = recaptcha_configured()
            && recaptcha_valid($_POST['g-recaptcha-response'] ?? null, 'login');

        if (!$credentialsValid) {
            $errors[] = 'Akun belum terdaftar, password salah, atau akun sedang tidak aktif.';
        } elseif (!$recaptchaValid) {
            $errors[] = recaptcha_configured()
                ? 'Verifikasi keamanan reCAPTCHA belum berhasil. Silakan coba lagi.'
                : 'reCAPTCHA belum dikonfigurasi pada server.';
        }
    }
...
<section class="auth-section auth-login-page">
    <div class="auth-shell">
        ...
        <div class="auth-panel glass-card">
            ...
            <?php if ($errors): ?>
                <div class="alert alert-danger small" role="alert"><?= implode('<br>', array_map('e', $errors)) ?></div>
            <?php endif; ?>
            ...
            <form method="post" novalidate data-recaptcha-action="login">
                ...
                <div class="mb-3">
                    <label class="form-label" for="login-identity">Username atau email</label>
                    <input id="login-identity" class="form-control" type="text" name="identity" value="<?= old('identity') ?>" placeholder="kamu@email.com" autocomplete="username" inputmode="email" required>
                </div>

                <div class="mb-2">
                    <label class="form-label" for="login-password">Password</label>
                    <div class="input-with-icon">
                        <input id="login-password" class="form-control" type="password" name="password" placeholder="Masukkan password" autocomplete="current-password" required>
                        <button type="button" class="toggle-password" aria-label="Tampilkan password"><i class="bi bi-eye"></i></button>
                    </div>
                </div>

                <div class="d-flex justify-content-end mb-4">
                    <a href="<?= e(page_url('forgot-password')) ?>" class="small text-link">Lupa password?</a>
                </div>

                <div class="recaptcha-placeholder mb-3">
                    <i class="bi bi-shield-check"></i>
                    Perlindungan reCAPTCHA v3 aktif.
                </div>
...
(() => {
    ...
    form.addEventListener('submit', function (event) {
        ...
        const oldWarning = form.querySelector('[data-recaptcha-warning]');
        if (oldWarning) oldWarning.remove();

        if (typeof grecaptcha === 'undefined') {
            const warning = document.createElement('div');
            warning.dataset.recaptchaWarning = '1';
            warning.className = 'alert alert-warning small';
            warning.textContent = 'Sistem keamanan belum siap. Tunggu sebentar lalu coba lagi.';
            form.prepend(warning);
            return;
        }

        grecaptcha.ready(function () {
            grecaptcha.execute('<?= e(RECAPTCHA_SITE_KEY) ?>', { action: 'login' })
                .then(function (token) {
                ...
                .catch(function () {
                    ...
                    warning.dataset.recaptchaWarning = '1';
                    warning.className = 'alert alert-warning small';
                    warning.textContent = 'Verifikasi keamanan gagal dimuat. Silakan coba lagi.';
                    form.prepend(warning);
                });
        });
    });
