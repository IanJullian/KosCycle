...
$user = current_user();
$hasReview = $user && current_role() === 'customer' ? $reviewRepo->exists((int) $user['id'], (int) $id) : false;
$canReview = $user && current_role() === 'customer' ? !$hasReview && $reviewRepo->canReview((int) $user['id'], (int) $id) : false;
$message = null;
...
if (is_post()) {
    ...
    if (!verify_csrf()) {
        ...
        if ($action === 'chat_seller') {
            ...
            try {
                $conversationId = (new ChatRepository())->startMarketplace((int) $_SESSION['user_id'], (string) current_role(), (int) $id);
...
<link rel="stylesheet" href="<?= e(APP_URL) ?>/assets/css/product-detail-v3.css">
...
<section class="section-padding page-section product-detail-page">
    <div class="container">
        ...
        <div class="row g-4 g-xl-5 align-items-start">
                ...
                <div class="product-detail-gallery glass-card overflow-hidden">
                    <div class="product-detail-main-image">
                        <?php if (!empty($product['images'])): ?>
                            <img id="product-main-image" src="<?= e(APP_URL . '/' . ltrim((string) $product['images'][0]['image_path'], '/')) ?>" alt="<?= e((string) $product['name']) ?>">
                        <?php else: ?>
                    ...
                    <?php if (count($product['images'] ?? []) > 1): ?>
                        <div class="product-detail-thumbnails" aria-label="Pilihan gambar produk">
                            <?php foreach ($product['images'] as $idx => $image): ?>
                                <?php $imageUrl = APP_URL . '/' . ltrim((string) $image['image_path'], '/'); ?>
                                <button type="button" class="product-thumb-button <?= $idx === 0 ? 'is-active' : '' ?>" data-image-src="<?= e($imageUrl) ?>" aria-label="Lihat gambar <?= $idx + 1 ?>">
                                    <img src="<?= e($imageUrl) ?>" alt="Thumbnail <?= $idx + 1 ?>">
                                </button>
                            <?php endforeach; ?>
                        </div>
                    <?php endif; ?>
            ...
            <div class="col-lg-5">
                <div class="product-detail-copy product-mobile-buy-panel">
                    <div class="product-detail-topline">
                        <span class="eyebrow"><?= e((string) $product['condition_label']) ?></span>
                        <span class="product-detail-stock <?= (int) $product['stock'] > 0 ? 'is-available' : 'is-empty' ?>"><?= (int) $product['stock'] > 0 ? 'Stok tersedia' : 'Stok habis' ?></span>
                    </div>
                    ...
                    <?php if ($message): ?><div class="alert alert-danger mt-4"><?= e($message) ?></div><?php endif; ?>
                    ...
                    <?php if (current_role() === 'customer'): ?>
                        <form method="post" class="product-detail-buy-box glass-card">
                            ...
                            <div><label class="form-label">Jumlah</label><input class="form-control" type="number" min="1" max="<?= max(1, (int) $product['stock']) ?>" name="quantity" value="1" <?= (int) $product['stock'] < 1 || $product['status'] !== 'available' ? 'disabled' : '' ?>></div>
        ...
        <div class="glass-card p-4 p-lg-5 mt-5 product-review-section" id="ulasan">
            <div class="d-flex justify-content-between align-items-end gap-3 flex-wrap">
                <div><span class="eyebrow"><i class="bi bi-star me-1"></i>Pengalaman customer</span><h2 class="mt-2 mb-1">Ulasan <em>produk.</em></h2><p class="text-muted mb-0">Rating hanya dapat diberikan setelah pesanan produk selesai.</p></div>
                <div class="product-review-summary"><strong><?= number_format((float) ($product['avg_rating'] ?? 0), 1) ?></strong><span><?= (int) ($product['review_count'] ?? 0) ?> ulasan</span></div>
            </div>

            <?php if ($canReview): ?>
                <div class="review-action-card"><div><strong>Pesananmu sudah selesai.</strong><span>Bagikan pengalamanmu agar customer lain mendapat gambaran.</span></div><a class="btn btn-primary" href="<?= e(page_url('review', ['product_id' => $product['id']])) ?>"><i class="bi bi-star me-1"></i>Tulis ulasan</a></div>
                ...
                <?php else: ?>
                    <?php foreach ($reviews as $r): ?>
                        <article class="product-review-item">
                            ...
                            <div class="flex-grow-1 min-w-0">
                                <div class="d-flex justify-content-between gap-2 flex-wrap"><div><strong><?= e((string) $r['full_name']) ?></strong><small><?= e((string) $r['created_at']) ?></small></div><span class="product-review-stars"><?php for ($i = 1; $i <= 5; $i++): ?><i class="bi <?= $i <= (int) $r['rating'] ? 'bi-star-fill' : 'bi-star' ?>"></i><?php endfor; ?></span></div>
