...
$firstCropX=(int)($firstImage['crop_x']??50);
$firstCropY=(int)($firstImage['crop_y']??50);
?>
                ...
                <div class="product-detail-main-image">
                    <?php if($firstImage):?>
                        <img
                            ...
                            src="<?=e(APP_URL.'/'.ltrim((string)$firstImage['image_path'],'/'))?>"
                            alt="<?=e((string)$product['name'])?>"
                            style="object-position:<?=$firstCropX?>% <?=$firstCropY?>%"
                        >
                    <?php else:?>
                ...
                <?php if(count($images)>1):?>
                    <div class="product-detail-thumbnails" aria-label="Pilihan gambar produk">
                        <?php foreach($images as $idx=>$image):?>
                            <?php
                            $imageUrl=APP_URL.'/'.ltrim((string)$image['image_path'],'/');
                            $cropX=(int)($image['crop_x']??50);
                            $cropY=(int)($image['crop_y']??50);
                            ?>
                            <button
                                type="button"
                                class="product-thumb-button <?=$idx===0?'is-active':''?>"
            ...
            <div class="product-detail-copy product-mobile-buy-panel">
                ...
                <div class="product-detail-rating-row">
                    <span class="product-rating-stars">
                        <?php for($i=1;$i<=5;$i++):?>
                            <i class="bi <?=$i<=round((float)$product['avg_rating'])?'bi-star-fill':'bi-star'?>"></i>
                        <?php endfor;?>
                    </span>
                ...
                <p class="product-detail-description">
                    <?=nl2br(e((string)($product['description']??'Produk layak pakai untuk dipakai kembali.')))?>
                </p>
                ...
                <?php if(current_role()==='customer'):?>
                    <form method="post" class="product-detail-buy-box glass-card">
                        ...
                        <div>
                            ...
                            <input
                                ...
                                name="quantity"
                                value="1"
                                <?=(int)$product['stock']<1||$product['status']!=='available'?'disabled':''?>
                            >
                        </div>
                    ...
                    <div class="product-detail-help-note">
                        <i class="bi bi-chat-heart"></i>
                        <span>Masih ragu? <strong>Chat seller</strong> untuk menanyakan kondisi atau detail produk.</span>
                    </div>

                <?php elseif(!$user):?>
                    <div class="d-flex gap-2 flex-wrap mt-4">
                        <a class="btn btn-primary" href="<?=e(page_url('login'))?>">Masuk untuk membeli</a>
                        <a class="btn btn-ghost" href="<?=e(page_url('register-customer'))?>">Buat akun customer</a>
                    </div>
                <?php endif;?>
            </div>
        </div>
        ...
        <div class="product-review-list">
            <?php if(!$reviews):?>
                <div class="product-review-empty">
                    ...
                    <span>Jadilah customer pertama yang berbagi pengalaman.</span>
                </div>
            <?php else:?>
                        ...
                        <div class="flex-grow-1 min-w-0">
                            <div class="d-flex justify-content-between gap-2 flex-wrap">
                                <div>
                                    ...
                                    <small><?=e((string)$r['created_at'])?></small>
                                </div>
...
(()=>{
    ...
    document.querySelectorAll('.product-thumb-button').forEach(button=>{
        button.addEventListener('click',()=>{
            ...
            main.src=src;
            main.style.objectPosition=`${button.dataset.cropX||50}% ${button.dataset.cropY||50}%`;

            document.querySelectorAll('.product-thumb-button').forEach(item=>{
