...
if ($action === 'messages') {
    ...
    try {
        ...
        // Membuka percakapan = membaca semua pesan lawan yang masih unread.
...
if (is_post()) {
    ...
    if (!verify_csrf()) {
        if ($isAjax) {
            ...
            echo json_encode(['ok' => false, 'message' => 'Sesi formulir tidak valid. Muat ulang halaman lalu coba lagi.'], JSON_UNESCAPED_UNICODE);
            exit;
        }
...
<section class="section-padding page-section chat-page">
    <div class="container">
        ...
        <div class="section-heading mb-4">
            ...
            <p class="text-muted mb-0"><?= $role === 'admin' ? 'Tangani pengaduan customer dan seller dari satu ruang kerja.' : 'Tanyakan produk langsung ke seller atau hubungi admin saat ada kendala.' ?></p>
        </div>
            ...
            <aside class="chat-sidebar glass-card">
                ...
                <div class="chat-conversation-list">
                    <?php if (!$conversations): ?>
                        <div class="chat-empty-list"><i class="bi bi-chat-dots"></i><strong>Belum ada percakapan</strong><span>Chat seller dari halaman produk atau buat pengaduan ke admin.</span></div>
                    <?php endif; ?>
                    <?php foreach ($conversations as $c): ?>
                        <a class="chat-conversation-item <?= $selected && (int) $selected['id'] === (int) $c['id'] ? 'is-active' : '' ?>" href="<?= e(page_url('chat', ['id' => $c['id']])) ?>">
            ...
            <section class="chat-main glass-card">
                <?php if (in_array($role, ['customer', 'seller'], true) && $newMode === 'support'): ?>
                    <div class="chat-empty-state chat-compose-state">
                        <span class="chat-hero-icon"><i class="bi bi-life-preserver"></i></span><h3>Buat pengaduan ke admin.</h3><p>Jelaskan masalahnya sedetail mungkin agar admin bisa menindaklanjuti dengan cepat.</p>
                        <form method="post" class="chat-support-form text-start"><input type="hidden" name="csrf_token" value="<?= e(csrf_token()) ?>"><input type="hidden" name="chat_action" value="start_support"><label class="form-label">Subjek pengaduan</label><input class="form-control mb-3" name="subject" maxlength="160" placeholder="Contoh: Pesanan #1024 belum diproses" required><label class="form-label">Kendala</label><textarea class="form-control" name="body" rows="6" maxlength="2000" placeholder="Ceritakan kendalanya di sini…" required><div class="d-flex gap-2 flex-wrap mt-3"><button class="btn btn-primary" type="submit">Kirim pengaduan <i class="bi bi-send ms-2"></i></button><a class="btn btn-ghost" href="<?= e(page_url('chat')) ?>">Batal</a></div></form>
                    </div>
                <?php elseif (!$selected): ?>
                    <div class="chat-empty-state"><span class="chat-hero-icon"><i class="bi bi-chat-heart"></i></span><h3><?= $role === 'admin' ? 'Pilih pengaduan.' : 'Pilih percakapan.' ?></h3><p><?= $role === 'admin' ? 'Daftar pengaduan pengguna tersedia di sisi kiri.' : 'Chat seller dimulai dari detail produk, sedangkan pengaduan admin tersedia di tombol Hubungi admin.' ?></p></div>
                <?php else: ?>
                    <div class="chat-main-head">
                        <div class="chat-main-person"><span class="chat-avatar chat-avatar-lg <?= $selected['type'] === 'support' ? 'is-support' : '' ?>"><i class="bi <?= $selected['type'] === 'support' ? 'bi-shield-check' : 'bi-person' ?>"></i></span><div><strong><?= e((string) ($selected['counterpart_name'] ?: 'Pengguna')) ?></strong><small><?= $selected['type'] === 'support' ? e((string) ($selected['subject'] ?: 'Pengaduan KosCycle')) : e((string) ($selected['product_name'] ?: 'Percakapan marketplace')) ?></small></div></div>
                        <?php if ($selected['type'] === 'support' && $selected['status'] === 'open' && $role === 'admin'): ?><form method="post" class="m-0"><input type="hidden" name="csrf_token" value="<?= e(csrf_token()) ?>"><input type="hidden" name="chat_action" value="close"><input type="hidden" name="conversation_id" value="<?= (int) $selected['id'] ?>"><button class="btn btn-ghost btn-sm" type="submit"><i class="bi bi-check2-circle me-1"></i>Tutup pengaduan</button></form><?php endif; ?>
                    </div>
...
(() => {
    ...
    const appendMessage = (message) => {
        ...
        if (!messageId || box.querySelector(`[data-message-id="${messageId}"]`)) return false;
    ...
    const updateReadReceipts = (readUpto) => {
        const maxReadId = Number(readUpto || 0);
    ...
    const poll = async () => {
        ...
        try {
            ...
            const response = await fetch(url.toString(), { headers: { 'X-Requested-With': 'XMLHttpRequest' }, cache: 'no-store' });
            if (!response.ok) return;
            const data = await response.json();
            if (!data.ok) return;

            let added = false;
            (data.messages || []).forEach((message) => { if (appendMessage(message)) added = true; });
            updateReadReceipts(data.read_upto || 0);

            // Hanya auto-scroll jika user memang sedang dekat bagian paling bawah.
            // Kalau sedang membaca pesan lama, posisi scroll dipertahankan.
            if (added && shouldStickToBottom) scrollBottom();
        } catch (_) {
            // Polling bersifat best-effort.
        } finally {
            polling = false;
    ...
    if (composer) {
        composer.addEventListener('submit', async (event) => {
            ...
            try {
                ...
                const data = await response.json();
                if (!response.ok || !data.ok) throw new Error(data.message || 'Pesan gagal dikirim.');
                ...
                status.textContent = error.message || 'Pesan gagal dikirim.';
            } finally {
