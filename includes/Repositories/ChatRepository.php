...
class ChatRepository
{
        ...
        if ($role === 'admin') {
            $stmt = db()->prepare(
                "SELECT
                    ...
                    c.last_message_at,
                    c.created_at,
                    COALESCE(cu.full_name, su.full_name, 'Pengguna') AS counterpart_name,
                    p.name AS product_name,
                    (
                        SELECT m.body
                        FROM chat_messages m
    ...
    {
        if ($role !== 'customer') {
            throw new RuntimeException('Chat seller hanya dapat dimulai oleh customer.');
        }

        $stmt = db()->prepare(
            "SELECT p.id, p.name, p.seller_id,
                    s.role AS seller_role, s.status AS seller_status
             FROM products p
        ...
        $stmt = db()->prepare(
            "INSERT INTO chat_conversations
                (type, customer_id, seller_id, product_id, status, created_by)
    ...
    public function startSupport(int $userId, string $role, string $subject, string $body): int
    {
        ...
        try {
            $stmt = $pdo->prepare(
                "INSERT INTO chat_conversations
                    (type, customer_id, seller_id, subject, status, created_by, last_message_at)
    ...
    public function sendMessage(int $conversationId, int $userId, string $role, string $body): int
    {
        ...
        if ($conversation['status'] !== 'open') {
            throw new RuntimeException('Percakapan sudah ditutup.');
        }
        ...
        try {
            ...
            $pdo->prepare(
                'UPDATE chat_conversations
                 SET last_message_at = NOW(), updated_at = CURRENT_TIMESTAMP
                 WHERE id = ?'
    ...
    public function closeConversation(int $conversationId, int $userId, string $role): void
    {
        ...
        if ($conversation['type'] !== 'support') {
            throw new RuntimeException('Chat marketplace tidak dapat ditutup dari fitur pengaduan.');
        }
        if ($role !== 'admin') {
            throw new RuntimeException('Hanya admin yang dapat menutup pengaduan.');
        }
